// components/ScheduleComposer.tsx
// Bottom sheet composer for schedule appointments
...
export default function ScheduleComposer({
  ...
  const [cTime, setCTime] = useState(initialTime);
  const [cDuration, setCDuration] = useState(initialDuration);
  const [cPriority, setCPriority] = useState(initialPriority);
  const [cDesc, setCDesc] = useState(initialDesc);
  const [saving, setSaving] = useState(false);
  const [r1h, setR1h] = useState(initialR1h);
  const [r30m, setR30m] = useState(initialR30m);
  const [r10m, setR10m] = useState(initialR10m);
  const [rAt, setRAt] = useState(initialRAt);

  // Update state when props change (for editing)
  useEffect(() => {
    if (show) {
      setCTitle(initialTitle);
      setCDate(initialDate);
      setCTime(initialTime);
      setCDuration(initialDuration);
      setCPriority(initialPriority);
      setCDesc(initialDesc);
      setR1h(initialR1h);
      setR30m(initialR30m);
      setR10m(initialR10m);
      setRAt(initialRAt);
    }
  }, [show, editingId, initialTitle, initialDate, initialTime, initialDuration, initialPriority, initialDesc, initialR1h, initialR30m, initialR10m, initialRAt]);
    ...
    <div className="fixed inset-0 z-20 flex items-end sm:items-center sm:justify-center">
      ...
      <div className="relative w-full sm:w-[480px] max-h-[90vh] overflow-y-auto rounded-t-2xl sm:rounded-2xl border border-[var(--border)] bg-[var(--surface-1)] shadow-xl">
        <div className="p-4 border-b border-[var(--border)] flex items-center justify-between">
          ...
          <button type="button" className="text-[var(--fg)]/70 text-sm hover:text-[var(--fg)]" onClick={() => !saving && onClose()} aria-label="Close">✕</button>
        </div>
        ...
        <div className="p-4 border-t border-[var(--border)] flex items-center justify-end gap-2">
          <button type="button" onClick={onClose} className="h-9 px-3 rounded-md border border-[var(--border)] bg-[var(--surface-2)] text-[var(--fg)]/80 hover:bg-[var(--surface-2)]/80">Cancel</button>
          <button type="button" disabled={saving || !cTitle.trim()} onClick={handleSave} className="h-9 px-4 rounded-md bg-emerald-600 text-white disabled:opacity-50 disabled:cursor-not-allowed hover:bg-emerald-500">{saving ? "Saving..." : "Save"}</button>
        </div>
      </div>
    </div>
  );
